...
for t in range(int(input())):
    n=int(input()) # 사이즈 입력
    maze=[[int(i) for i in input()]for _ in range(n)] # 미로입력 (입력값이 띄어쓰기 없이 들어오는경우)
    # maze=[[*map(int,input().split())]for _ in range(n)] # 미로입력 (입력값이 띄어로 들어오는경우)
    ...
    visit=[[[0]*2 for _ in range(n)]for _ in range(n)] # 방문 여부를 저장하는 변수 visit[y좌표][x좌표][벽 부술수 있는 횟수]
